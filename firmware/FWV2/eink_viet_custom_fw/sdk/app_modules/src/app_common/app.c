...
static struct gapc_param_update_cmd* app_easy_gap_param_update_msg_create_high(uint8_t conidx)
{
    // Allocate a message for GAP
    if (param_update_cmd[conidx] == NULL)
    {
        ...
        cmd->intv_max = user_connection_param_conf_high.intv_max;
        cmd->intv_min = user_connection_param_conf_high.intv_min;
        cmd->latency = user_connection_param_conf_high.latency;
        cmd->time_out = user_connection_param_conf_high.time_out;
        cmd->ce_len_min = user_connection_param_conf_high.ce_len_min;
        cmd->ce_len_max = user_connection_param_conf_high.ce_len_max;
    }
    return param_update_cmd[conidx];
}
#if !defined (__DA14531_01__)
...
{
    struct gapc_param_update_cmd* cmd;
    cmd = app_easy_gap_param_update_msg_create_high(conidx);
