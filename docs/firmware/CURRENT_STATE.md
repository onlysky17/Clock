...
## HL22A validated state - 2026-07-10
...
Runtime:
- E5 completion manifest:
  state=02
  chunks=339
  bytes=4736
  CRC16=6065
- E5 status manifest matched completion.
- Legacy E3 regression passed.
- HL20A regression passed:
  E2 E0 A1 and E2 03 F0.
- E4 close passed.
- Post-close E5 returned session-required status 01.
- Final result:
  HL22A PASS END-TO-END.

Safety:
- Session-bound transfer ID and CRC16 validation active.
...
## HL24A validated state - 2026-07-10

Status:
PASS END-TO-END.

Runtime:
- Real 296x128 canvas packed to raw 128x296.
- Exactly 4736 bytes produced.
- 339 E5 chunks transmitted.
- Dynamic CRC16 calculated from the actual canvas payload.
- Completion manifest matched:
  state=02,
  chunks=339,
  bytes=4736,
  CRC matched the web-calculated canvas CRC.
- Final result:
  PASS END-TO-END.

Safety:
- Existing HL22A firmware only.
- Web-only validation.
