...
for (const marker of ['mainImageFile', 'mainImageFrameMode', 'value="manual"', 'Crop tay', 'mainImageManualViewport', 'mainImageCropZoom', 'mainImageCropReset', 'pointerdown', 'pointermove', 'setPointerCapture', 'panManualCropByViewportDelta', 'computeManualCropPlacement', 'resetManualCrop(false)', 'mainImageRotation', 'mainImageOutput', 'mainImageThresholdCard', 'mainImageDitherCard', 'role="button"', "event.key === 'Enter'", "event.key === ' '", 'data-selected', 'createImageTransferPlan(frame', 'session.runExclusive', 'session.subscribeState', 'Đang gửi ảnh', 'Đang kiểm tra ảnh', 'Đang hiển thị ảnh', 'Hoàn tất']) assert.ok(imageTabSource.includes(marker), `Missing image tab contract: ${marker}`);
