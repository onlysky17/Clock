...
mustMatch('solar text uses bitmap', /drawBitmapText\(solar,5,8,\{scale:2\}\)/);
mustMatch('AL text uses bitmap', /const lunarText=`AL \$\{pad2\(lunar\.day\)\}\/\$\{pad2\(lunar\.month\)\}/);
mustMatch('month title uses bitmap', /drawBitmapText\(`THANG \$\{now\.getMonth\(\)\+1\}\/\$\{now\.getFullYear\(\)\}`/);
